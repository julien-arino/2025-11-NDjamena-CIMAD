...
\subtitle{CIMAD N'Djaména 2025 -- Cours 01}
...
\begin{frame}{Quelques jalons en épidémiologie mathématique}
\framesubtitle{(opinion personnelle baisée)}
...
\begin{frame}{Modèle compartimental}
    ...
    \begin{center}
        \begin{tikzpicture}[
            node distance=3cm,
            compartment/.style={rectangle, draw, fill=blue!20, rounded corners, minimum height=1.25cm, minimum width=3cm, text width=3cm, align=center},
            arrow/.style={-latex, thick}]
            %% Les sommets
            \node[compartment] (C1) {Compartiment 1 ($x_1$)};
            \node[compartment, right=1.5cm of C1] (C2) {Compartiment 2 ($x_2$)};
            \node[compartment, right=1.5cm of C2] (C3) {Compartiment 3 ($x_3$)};
            %% Sommets vides pour faire des flèches
            \node[above=1cm of C1] (entreeC1) {};
            \node[above=1cm of C3] (sortieC3) {};
            %% Les flèches
            ...
            \draw[arrow] (C3) to [bend left] node[below] {$k_{23}$} (C2);
            \draw[arrow] (entreeC1) to node[left] {Entrée} (C1);
            \draw[arrow] (C3) to node[right] {Sortie} (sortieC3);
        \end{tikzpicture}
    \end{center}
    \vfill
    \textbf{Hypothèse clé:} La population dans chaque compartiment est instantanément et parfaitement mélangée
\end{frame}
...
\begin{frame}{Histoire ancienne des idées compartimentales}
    \begin{itemize}
        ...
        \item \textbf{John A. Jacquez (1922-2002):} Pionnier de la théorie mathématique
        \begin{itemize}
...
\begin{frame}{Les éléments de base}
    \begin{itemize}
        ...
        \end{itemize}
        \vfill
        \item \textbf{Variables d'état ($x_i(t)$):} La quantité ou concentration de matière dans le compartiment $i$ au temps $t$. L'état du système est le vecteur $\vect{x}(t) = (x_1(t), x_2(t), \dots, x_n(t))$
        \vfill
        \item \textbf{Flux:} Taux de transfert de matière entre compartiments ou entre un compartiment et le monde extérieur (environnement)
        \begin{itemize}
            \item $f_{ij}$ taux de flux du compartiment $j$ vers le compartiment $i$
            \item $f_{0j}$ taux de flux du compartiment $j$ vers l'environnement (excrétion/sortie)
            \item $f_{i0}$ taux de flux de l'environnement vers le compartiment $i$ (entrée)
        \end{itemize}
        \vfill
...
\begin{frame}{}
  \begin{minipage}{0.4\textwidth}
    ...
    \begin{tikzpicture}[scale=1.5, transform shape]
      ...
      %% Flows
      \path [line, very thick] (h1) to node [midway, right] (TextNode) {$f_{i0}(t)$} (S);
  ...
  \begin{minipage}{0.55\textwidth}
    \begin{itemize}
      \item $x_i\geq 0$ taille du compartiment, i.e., quantité de matériau cinétiquement homogène présent dans $i$
      \item $f_{ij}$ et $f_{ji}$ coefficients/fonctions de transfert
      \item $f_{0i}$ coefficient/fonction d'excrétion
      \item $f_{i0}(t)$ entrées depuis l'extérieur du système
    \end{itemize}
  \end{minipage}
  \vfill
...
\begin{frame}{Formulation mathématique: le système EDO}
    La dynamique d'un système compartimental est décrite par un système d'EDO du premier ordre

    \begin{definition}[Équation de bilan]
        Pour chaque compartiment $i=1,\ldots,n$, le taux de changement de la variable d'état $x_i$ est donné par:
        \begin{multline*}
          \frac{dx_i}{dt} = \text{Somme de tous les flux entrants dans } C_i \\
          - \text{Somme de tous les flux sortants de } C_i
        \end{multline*}
    \end{definition}
    \vfill
    Par abus de language, on dit souvent $x_i$ pour le compartiment $i$
\end{frame}

\begin{frame}{Formulation mathématique}
    Pour un système à $n$ compartiments:
    \[
      x_i' = f_{i0}(t) + \sum_{j=1, j\neq i}^{n} f_{ij}(\vect{x}) - \sum_{j=0, j\neq i}^{n} f_{ji}(\vect{x}),\qquad i=1,\ldots,n
    \]
    où:
    \begin{itemize}
...
\begin{frame}{Exemple: un modèle simple à deux compartiments}
    Un médicament est injecté dans le sang puis se déplace vers les tissus et est également éliminé du sang
    ...
    \begin{center}
        \begin{tikzpicture}[
            ...
            \node (env_out) [below=1cm of C1] {};

            \draw[arrow] (env_in) to node[left] {$I(t)$} (C1); 
            \draw[arrow] (C1) to[bend left] node[above] {$k_{21}$} (C2);
            \draw[arrow] (C2) to[bend left] node[below] {$k_{12}$} (C1);
            \draw[arrow] (C1) to node[left] {$k_{01}$} (env_out);
        \end{tikzpicture}
    \end{center}

    Supposant une cinétique linéaire du premier ordre ($f_{ij} = k_{ij}x_j$)
    \begin{align*}
        x_1' &= I(t) + k_{12}x_2 - k_{21}x_1 - k_{01}x_1 \\
        x_2' &= k_{21}x_1 - k_{12}x_2
    \end{align*}
\end{frame}

\begin{frame}{Modèles compartimentaux linéaires}
    \begin{itemize}
...
\begin{frame}{Modèles compartimentaux non linéaires}
    \begin{itemize}
...
\begin{frame}{Espace d'état}
    ...
    $$ \Omega = \IR_+^n = \{ \vect{x} \in \IR^n : x_i \ge 0 \text{ pour tout } i=1, \dots, n \} $$
\end{frame}

\begin{frame}{Positivité des solutions}
  Une propriété cruciale d'un modèle compartimental bien posé est que si le système commence avec des conditions initiales non négatives, $\vect{x}(0) \in \Omega$, alors la solution $\vect{x}(t)$ doit rester dans $\Omega$ pour tout $t > 0$
  \vfill
  C'est l'invariance de $\IR_+^n$ sous le flot du système
\end{frame}

\begin{frame}{Condition pour la positivité des solutions}
    \begin{theorem}[Condition de positivité, souvent attribuée à Jacquez]
        Considérons le système $\bx' = \vect{f}(\vect{x})$ avec $\vect{x}(0) \ge 0$. L'orthant positif $\IR_+^n$ est positivement invariant sous le flot de $\bf$ si et seulement si pour chaque $i=1, \dots, n$
        $$ f_i(\vect{x}) \ge 0 \quad \text{lorsque} \quad \vect{x} \in \IR_+^n \text{ et } x_i=0. $$
    \end{theorem}
    \vfill
...
\begin{frame}{Exemple épidémiologique: Le modèle SIR}
    ...
    \begin{align*}
        ...
        \frac{dR}{dt} &= \gamma I
    \end{align*}
    Ici, $S(t)+I(t)+R(t) = N$ est constant. L'espace d'état est le simplexe $\Delta = \{(S,I,R) \in \IR_+^3 : S+I+R=N\}$.
\end{frame}
...
\begin{frame}{Équilibres et stabilité}
    ...
    \begin{itemize}
        \item \textbf{Équilibre sans maladie (ÉSM):}
        Ceci correspond à l'absence de maladie. Nous posons $I=0$.
        $$ I^* = 0 \implies \dot{S}=0, \dot{I}=0, \dot{R}=0 $$
        L'ÉSM est tout point $(S, 0, R)$ où $S+R=N$. Nous le désignons typiquement par $E_0 = (N, 0, 0)$.
...
\begin{frame}{Stabilité globale et fonctions de Lyapunov}
    ...
    \vfill
    Trouver une fonction de Lyapunov est un art, mais pour de nombreux modèles épidémiologiques, elles peuvent être construites pour prouver la stabilité globale soit du ÉSM (quand $R_0 < 1$) soit de l'EE (quand $R_0 > 1$). Cela fournit des garanties beaucoup plus fortes sur le comportement du système
\end{frame}
...
\begin{frame}
Les ÉSM \eqref{eq:DFE_KMK} de \eqref{sys:KMK} ne sont pas \defword{isolés} : tout voisinage (ouvert) d'un équilibre contient une infinité d'autres équilibres
\vfill
\begin{center}
    \begin{tikzpicture}
...
\end{proposition}
\vfill
Cela signifie en particulier que considérer la matrice jacobienne de \eqref{sys:KMK} au ÉSM \textbf{n'a aucun sens} !
\end{frame}
...
\vfill
Le Théorème 2 de \cite{VdDWatmough2002} concernant la LAS, d'autre part, a une hypothèse (hypothèse A5) selon laquelle l'ÉSM doit être \emph{localement asymptotiquement stable}, avec l'hypothèse que toutes les valeurs propres de la linéarisation près d'un équilibre sans maladie ont des parties réelles négatives
\vfill
...
\vfill
\begin{center}
\includegraphics[width=\textwidth]{\Sexpr{knitr::plot_crop( fig_chunk('KMK_SI_plane', 'pdf'))}}
\end{center}
\end{frame}
...
\begin{frame}{}
  \begin{theorem}[Epidemic or no epidemic?]
    Soient $(S(t),I(t))$ une solution de \eqref{sys:KMK_2d} et $\R_0$ définis par
...
@
\begin{center}
\includegraphics[width=0.8\textwidth]{\Sexpr{knitr::plot_crop(fig_chunk('KMK_R0eq1dot5', 'pdf'))}}
\end{center}
\end{frame}
...
\begin{frame}{Quelques valeurs types de $\R_0$}
  $\R_0$ peut être estimé en utilisant les données (table du livre  de Anderson \& May)
  \vfill
  \begin{center}
  \begin{tabular}{llcc}
  \hline 
  Infection & Lieux & Période & $\R_0$ \\
  \hline
  Rougeole & Cirencester, Angleterre & 1947-50 & 13-14 \\
  & Angleterre et Pays de Galle & 1950-68 & 16-18 \\
  & Kansas, USA & 1918-21 & 5-6 \\
  & Ontario, Canada & 1912-3 & 11-12 \\
  & Willesden, Angleterre & 1912-3 & 11-12 \\
  & Ghana & 1960-8 & 14-15 \\
  & Est du Nigéria & 1960-8 & 16-17 \\
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{}
  ...
  \end{equation}
  Exprimons \eqref{eq:KMK_final_size_step1} et \eqref{eq:KMK_final_size_step2} en terme de $-\hat I$ et égalisons
  $$
...
\begin{frame}{Relation de taille finale}
\begin{theorem}[Relation de taille finale]
  Soient $(S(t),I(t))$ une solution de \eqref{sys:KMK_2d} et $\R_0$ définis par \eqref{eq:R0_KMK}
  \vskip0.5cm
  Le nombre $S(t)$ d'individus susceptibles est une fonction décroissante et sa limite $S_\infty$ est l'unique solution dans $(0,S_0)$ de l'équation transcendante
...
\begin{frame}{L'équation (transcendante) de taille finale}
  Réécrivons l'équation de taille finale
  \begin{equation}
...
\begin{frame}[fragile]{Une figure avec toute l'information}
<<KMK_final_size_0p8>>=
N0 = 1000
I0 = 1
S0 = N0-I0
...
\begin{frame}{$\R_0=0.8$}
\begin{center}
  \includegraphics[width=\textwidth]{\Sexpr{knitr::plot_crop(fig_chunk('KMK_final_size_0p8', 'pdf'))}}
\end{center}
\end{frame}
...
\begin{frame}{$\R_0=2.4$}
  \begin{center}
    \includegraphics[width=\textwidth]{\Sexpr{plot_crop(fig_chunk('KMK_final_size_2p5', 'pdf'))}}
  \end{center}
\end{frame}

\begin{frame}[fragile]{Un peu plus joli}
<<KMK_attack_rate>>=
values = expand.grid(
...
print(p)
@
(nécessite les librairies \code{lattice}, \code{viridis} et \code{latex2exp})
\end{frame}

\begin{frame}{Taux d'attaque (en \%)}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{\Sexpr{plot_crop(fig_chunk('KMK_attack_rate', 'pdf'))}}
  \end{center}
\end{frame}
...
\begin{frame}
Supposons que $\bE_0$ soit un équilibre sans maladie (ÉSM) localement stable du système sans maladie, c'est-à-dire, un EP de
\begin{align*}
...
\end{equation}
\begin{itemize}
\item Si $\mathcal{R}_0<1$, l'ÉSM $\bE_0$ est un EP localement asymptotiquement stable de \eqref{sys:SIR_general}
\item Si $\mathcal{R}_0>1$, l'ÉSM $\bE_0$ de \eqref{sys:SIR_general} est instable
\end{itemize}
\end{theorem}
\vfill
...
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
\subsection{Une variation sur le modèle SLIAR}
\newSubSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_vqpscpvqpscpvqps.jpeg}

\begin{frame}{Le modèle SLIAR}
\bbullet Papier que nous avons déjà vu: Arino, Brauer, PvdD, Watmough \& Wu. \href{http://dx.doi.org/10.1098/rsif.2006.0112}{Simple models for containment of a pandemic}, \emph{Journal of the Royal Society Interface} (2006)
\vfill
...
\begin{frame}
Ici, $\bS=S$, $\bI=(L,I,A)^T$ et $\bR=R$, $m=1$, $n=3$ et donc 
$$
...
\]
\vfill
Supposons que $\bI_0=(0,I_0,0)$, alors
\[
\ln\left(
\frac{S_0}{S_\infty}
\right)
...
%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%
\subsection{Resistance aux antiviraux}
\newSubSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_vqpscpvqpscpvqps.jpeg}
...
\begin{frame}
Étend le modèle SLIAR pour tenir compte des durées de stade non exponentiellement distribuées (voir cours 03)
\end{frame}
...
Ici $D$ signifie \emph{détecté}, $U$ est \emph{non détecté} (\emph{undetected})
\vfill
\centering
...
\end{itemize}
\vfill
\bbullet Au 2020-01-29, le virus était présent dans toutes les provinces de la Chine continentale
\end{frame}
...
\begin{minipage}{0.7\textwidth}
  ...
  \end{subequations}
  \vskip1cm
  Considérons le problème de Cauchy constitué de \eqref{sys:SIRS_base} muni des conditions initiales $S(0)=S_0\geq 0$, $I(0)=I_0\geq 0$ et $R(0)=R_0\geq 0$
  \vskip1cm
...
\begin{frame}{Naissance et mort sont \emph{relatifs}}
  Remarquons que les notions de \emph{naissance} et \emph{mort} sont relatifs à la population considérée
  \vfill
...
\]
avec la condition initiale $N(0)=N_0$
\vfill
...
\vfill
Lorsque $I^\star=0$, substituer $I^\star=0$ dans \eqref{sys:SIRS_EP_dR} implique que $R^\star=0$ et, à son tour, substituer $I^\star=R^\star=0$ dans \eqref{sys:SIRS_EP_dR} donne $S^\star=b/d$. Ceci donne l'équilibre sans maladie (ÉSM)
...
$\R_0$ est la surface dans l'espace des paramètres où l'ÉSM perd sa LAS
\vfill
Pour trouver $\R_0$, nous étudions donc la LAS du ÉSM
\vfill
...
\begin{frame}
La LAS de l'ÉSM dépend du signe des parties réelles des valeurs propres de \eqref{eq:SIRS_Jacobian} à ce point d'équilibre, donc nous évaluons
\begin{equation}
...
\end{equation}
\vfill
Matrice bloc triangulaire supérieure $\implies$ valeurs propres $-d<0$, $-(d+\nu)<0$ et $\beta S^\star-(d+\gamma)$ 
\vfill
$\implies$ SAL de l'ÉSM est déterminé par le signe de $\beta S^\star-(d+\gamma)$
\end{frame}


\begin{frame}{Signe de $\beta S^\star-(d+\gamma)$}
Rappelons qu'au ÉSM \eqref{eq:SIRS_DFE}, $S^\star=b/d$, donc
\[
...
\end{equation}
\vfill
(On écrit parfois $b/d=N^\star$ pour mettre en évidence la population totale, et écrivons par conséquent $\R_0=\beta N^\star/(d+\gamma)$)
\end{frame}
...
\begin{frame}{Le PEE}
L'\defword{équilibre endémique} (PEE) de \eqref{sys:SIRS} est
...
\end{multline}
\vfill
Remarquons que $\bE_\star$ n'est \defword{pas biologiquement pertinent} lorsque $\R_0\leq 1$
\end{frame}


\begin{frame}
\begin{theorem}\label{th:SIRS_LAS_DFE}
Définissons le nombre de reproduction élémentaire comme
\begin{equation}\tag{\ref{eq:SIRS_R0}}
\R_0 = \frac\beta{d+\gamma}\ N^\star
\end{equation}
et considérons l'EP de \eqref{sys:SIRS}: l'ÉSM
...
\vskip0.5cm
\begin{itemize}
\item Si $\R_0<1$, alors $\bE_0$ est LAS et $\bE_\star$ n'est pas biologiquement pertinent
\item Si $\R_0>1$, alors $\bE_0$ est instable et $\bE_\star$ est pertinent biologiquement
\end{itemize}
\end{theorem}
\end{frame}
...
\vfill
Avec encore plus de sang, de sueur et de larmes, nous pouvons montrer que le résultat est \emph{global}
\vfill 
Nous exprimons cela dans la diapositive suivante
\end{frame}

\begin{frame}
\begin{theorem}\label{th:SIRS_GAS_behaviour}
Soient le nombre de reproduction de base \eqref{eq:SIRS_R0}, l'ÉSM \eqref{eq:SIRS_DFE} et le PEE \eqref{eq:SIRS_EEP}
\vskip1cm
\begin{itemize}
\item Si $\R_0<1$, alors $\bE_0$ est globalement asymptotiquement stable (GAS) et $\bE_\star$ n'est pas pertinent biologiquement
\item Si $\R_0>1$, alors $\bE_0$ est instable and $\bE_\star$ is GAS
\end{itemize}
\end{theorem}
\vfill
En d'autres termes
\begin{itemize}
\item quand $\R_0<1$, alors toutes les solutions vont vers l'ÉSM, la maladie s'\defword{éteint}
...
\begin{frame}{}
  \begin{center}
    \includegraphics[width=\textwidth]{\Sexpr{plot_crop(fig_chunk('SIRS_one_sim_prevalence', 'pdf'))}}
  \end{center}
\end{frame}
...
\begin{frame}{}
  \begin{center}
    \includegraphics[width=\textwidth]{\Sexpr{plot_crop(fig_chunk('SIRS_3_sims_prevalence', 'pdf'))}}
  \end{center}
\end{frame}
...
\begin{frame}
  \begin{center}
    \includegraphics[width=\textwidth]{\Sexpr{plot_crop(fig_chunk('SIRS_bifurcation_R0', 'pdf'))}}
  \end{center}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Immunité collective dans le modèle SIRS}
\newSubSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_vqpscpvqpscpvqps.jpeg}
...
\vfill
L'ÉSM est affecté, cependant; en conséquence, le nombre de reproduction l'est aussi
\end{frame}

\begin{frame}{L'ÉSM pour le modèle SIRS avec vaccination}
...
\begin{frame}
En conséquence, L'ÉSM prend la forme
...
\end{equation}
\vfill
Substituant \eqref{eq:SIRS_vacc_DFE} dans la valeur propre $\beta S^\star-(d+\gamma)$ qui détermine la stabilité de l'ÉSM, on obtient
\begin{align*}
...
Mathématiquement, $\R_0$ est un paramètre de bifurcation agrégeant certains des paramètres du modèle et tel que l'équilibre sans maladie (ÉSM) perd sa stabilité asymptotique locale lorsque $\R_0=1$ est franchi de gauche à droite
\vfill
\begin{itemize}
\item
En conséquence, $\R_0$ se trouve en considérant le spectre de la matrice jacobienne du système évaluée au ÉSM
\vfill
\item
...
Soit $x_0$ un ÉSM du système, c'est-à-dire, une solution d'équilibre (localement asymptotiquement) stable du modèle sans maladie, c'est-à-dire, le système restreint à $X_s$. Nous n'avons pas besoin de supposer que le modèle a un ÉSM unique
\vfill
...
\begin{frame}{Stabilité du ÉSM en fonction de $\mathcal{R}_0$}
...
\begin{itemize}
\item si $\mathcal{R}_0<1$, alors L'ÉSM est LAS
\item si $\mathcal{R}_0>1$, L'ÉSM est instable
\end{itemize}
\end{theorem}
\vfill
...
$\mu$ paramètre de bifurcation tel que $\mathcal{R}_0<1$ pour $\mu<0$ et $\mathcal{R}_0>1$ pour $\mu>0$ et $x_0$ ÉSM pour toutes les valeurs de $\mu$ et considérons le système
\begin{equation}
\label{eq:sys_PvdDW}
x'=f(x,\mu)
\end{equation}
...
\begin{frame}
\frametitle{Exemple du modèle SLIRS \eqref{sys:SLIRS}}
Les variations des variables infectées dans \eqref{sys:SLIRS} sont décrites par
\begin{align*}
...
\right|_{(S,I,R)=\bE_0}
\]
les valeurs des dérivées partielles de la fonction d'incidence en l'ÉSM $\bE_0$
\vfill
Calculer les matrices jacobiennes des vecteurs $\F$ et $\V$ à l'ÉSM $\bE_0$
\begin{equation}
F=\left(
\begin{matrix}
f_L^{\,\star} & f_I^{\,\star} \\
...
Alors
\begin{itemize}
\item si $\R_0<1$, l'ÉSM est LAS
\item if $\R_0>1$, l'ÉSM est instable
\end{itemize}
\end{theorem}
\vfill
...
\]
\begin{itemize}
\item Incidence en action de masse (contacts fréquence-dépendants):
 \[
f_I^{\,\star}=\beta\bar S \Rightarrow\R_0 =
\frac{\epsilon\beta N}{(\epsilon+d)(\gamma+d)} 
\]
\item Incidence standard (contacts proportion-dépendants):
\[
...
\begin{frame}
\frametitle{Liens entre modèles de type SLIRS}
{ %\footnotesize
\begin{align*}
S\pprime&=b+\nu R-dS-f(S,I,N) \\
...
\hline
SLIR & SLIRS où $\nu=0$ \\
SLIS & Limite SLIRS lorsque $\nu\to\infty$ \\
SLI & SLIR où $\gamma=0$ \\
SIRS & Limite SLIRS lorsque $\varepsilon\to\infty$ \\
SIR & SIRS où $\nu=0$ \\
SIS & Limite SIRS lorsque $\nu\to\infty$ \\
& Limite SLIS lorsque $\varepsilon\to\infty$ \\
SI & SIS où $\nu=0$ 
\end{tabular}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{Valeurs de $\R_0$}
$(\bar S,\bar I,\bar N)$ valeurs de $S,I$ et $N$ à l'ÉSM. 
Notons $\bar f_I=\partial f/\partial I(\bar S,\bar I,\bar N)$
\begin{center}
\begin{tabular}{cc}
Modèle & $\R_0$ \\
\midrule
...
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Un meilleur modèle de vaccination?}
\newSubSectionSlide{FIGS-slides-admin/Gemini_Generated_Image_vqpscpvqpscpvqps.jpeg}
...
\begin{frame}{Personnes à lire pour les modèles de paludisme}
...
\vfill
Les mathématiques deviennent plus compliquées
\end{frame}
...
\begin{frame}{Le modèle de Codeço}
...
\end{minipage}
\vfill
Notez que la flèche pointillée de $I$ à $B$ n'est pas un flux à proprement parler: les individus ne se convertissent pas en \emph{vibrio cholerae}
\end{frame}
...
\begin{frame}
  \begin{center}
    ...
    \def\horzskip{*3}
    \begin{tikzpicture}[scale=1, transform shape]
      \node [rectangle, fill=gray!10, text=black] at (-1\horzskip,0\vertskip) (S_H) {Humains susceptibles};
      \node [rectangle, fill=gray!10, text=black] at (1\horzskip,0\vertskip) (I_H) {Humains infectieux};
      \node [rectangle, fill=gray!10, text=black] at (-1\horzskip,2\vertskip) (S_A) {Animaux susceptibles};
      \node [rectangle, fill=gray!10, text=black] at (1\horzskip,2\vertskip) (I_A) {Animaux infectieux};
      \node [rectangle, fill=gray!10, text=black] at (0\horzskip,1\vertskip) (W) {Oo/cystes vivant dans l'eau};
      %% Flows 
      \path [line, very thick] (S_H.south east) to node [midway, below] (TextNode) {Transmission P2P} (I_H.south west);
      \path [line, dashed, very thick] (S_H.north east) to node [near end, above] (TextNode) {conversion des oo/cystes en infection} (I_H.north west);
      \path [line, very thick, bend left] (I_H) to node [midway, below] (TextNode) {guérison} (S_H);
      \path [line, very thick] (S_A.north east) to node [midway, above] (TextNode) {Transmission A2A} (I_A.north west);
      \path [line, very thick] (I_A.south west) to node [midway, below] (TextNode) {guérison} (S_A.south east);
      %% Flows of W
      \path [line, dashed, very thick] (W) to node [midway, above] (TextNode) {Mort oo/cystes dans l'eau} (-2\horzskip,1\vertskip);
      \path [line, dashed, very thick] (W) to node [near start, left] (TextNode) {taux de ramassage} (0\horzskip,0.35\vertskip);
      \path [line, dashed, very thick] (I_A) to node [midway, below] (TextNode) {taux de déposition} (W);
    \end{tikzpicture}    
  \end{center}  
\end{frame}
...
\begin{frame}{Le modèle complet}
  ...
  \end{subequations}
  \vfill
  Considéré avec $N_A=S_A+I_A$ et $N_H=S_H+I_H$ constants
\end{frame}

\begin{frame}{Simplified model}
  Puisque $N_A$ et $N_H$ sont constants, \eqref{sys:WaterHamilton_etal} se simplifie:
  \begin{subequations}
  ...
  \end{subequations}
  \vfill
  Trois EP: ÉSM $(0,0,0)$; maladie endémique chez les humains en raison de la transmission H2H; endémique à la fois chez H et A en raison de W
\end{frame}
...
\begin{frame}
  ...
  Soit
  \begin{equation}
    \R_{0A} = \frac{\beta_A}{\gamma_A}N_A\quad\text{et}\quad
    \R_{0H} = \frac{\beta_H}{\gamma_H}N_H
  \end{equation}
  \vfill
  \begin{itemize}
    \item ÉSM LAS si $\R_{0A}<1$ et $\R_{0H}<1$, instable si $\R_{0A}>1$ ou $\R_{0H}>1$
    \item Si $\R_{0H}>1$ et $\R_{0A}<1$, \eqref{sys:WaterHamilton_etal_simplified} va vers le PE avec endémicité seulement chez les humains
    \item PE endémique avec A et H requiert $\R_{0A}>1$ et $\R_{0H}<1$
  \end{itemize}
...
\vfill
\bbullet Cela peut simplifier certains calculs
\vfill
...
\vfill
\bbullet Cela peutsimplifier certains calculs
\vfill
...
\begin{frame}[fragile]\frametitle{Au sujet de ce fichier \code{R}}
